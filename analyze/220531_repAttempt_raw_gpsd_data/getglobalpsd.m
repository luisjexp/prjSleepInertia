function gpsd = getglobalpsd(cog_test)
    % provides same output as psd compare function, but in table format
    % i built it so that i can understand the analysis better.
    psd = extractpsd(cog_test);
    
    BL_global = getgpsd(psd.baseline, nan, 'baseline');
    light_global = [];
    cnt_global = [];
