function [wpli_tble,betw,pathl_tble,eff,ecc,rad,strg_tble, modul_tble] = getnetwork(test_folder)
% my shortened version of extracting_wpli. its an attempt to understand it
% and also convert the output to a table format

S = subjects(test_folder);
load(strcat(S.path_data, 'B_cond.mat'));
load(strcat(S.path_data, 'A_cond.mat'));
    ...
    for r = run_list_light(:)'
        ...
        if r > length(data.(night_str))
            ...
            if ~isempty(data.(night_str)(r).wpli)
                ...
                %clust.condA.(run_str)(1:4,1:9,i) = mean(data.(sprintf('night%d',night_no))(j).wpli.clusteringcoeff,3);
    ...
    for r = run_list_control(:)'
        ...
        if r > length(data.(night_str))
            ...
            if ~isempty(data.(night_str)(r).wpli)
                ...
                %clust.condB.(run_str)(1:4,1:9,i) = mean(data.(sprintf('night%d',night_no))(j).wpli.clusteringcoeff,3);
...
wpli_tble = create_tbl(wpli); 
pathl_tble = create_tbl(pathl); 
modul_tble = create_tbl(modul);
strg_tble = create_tbl(strg);

end


% Convert Structure to table
function  [tble] = create_tbl(netstruct) 
    ...
    bl_tbl     = cell2table(cat_table(netstruct.baseline,'baseline',nan));
