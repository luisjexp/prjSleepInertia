...
[wpli,betw,pathl,eff,ecc,rad,strg] = extracting_wpli_light_copy_scratch(A_cond,B_cond);
...
n_sub = 12;
% n_sub = 11;
[output,pow_baseline,powA,powB] = psd_comparison(psd,1,251,n_sub,1:23);
% a_sub = [1,2,3,4:7,9:12];
a_sub = [1:6,8:10]; % this was originally uncommented. but im using all 12 subjects 
a_sub = 1:12 %ill use all 12 subjects
n = length(a_sub);
mm = strg; 
...
%% Figure 1
%%% panel A
% this one is weird. they compare baseline psds with the average of the two
% first two runs; this isnt what they did in the paper
figure;
...
for i = 1:4
    ...
    subplot(1,4,i),boxplot(tb(3,:),chan,'style','map','electrodes','on');
