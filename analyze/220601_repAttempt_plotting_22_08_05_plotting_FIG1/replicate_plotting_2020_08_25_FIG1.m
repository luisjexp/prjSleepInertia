...
[~,~,~,~,~,~,her_mm] = extracting_wpli_light_copy_scratch(A_cond,B_cond, S.path_data_cog_test);
...
for i = 1:4    
    ...
    her_bs(:,i,1:n)= her_mm.baseline(i,:,a_sub);
    her_p1(:,i,1:n)= her_mm.condB.run1(i,:,a_sub);
    her_p2(:,i,1:n)= her_mm.condB.run2(i,:,a_sub);
    her_p3(:,i,1:n)= her_mm.condB.run3(i,:,a_sub);
    her_p4(:,i,1:n)= her_mm.condB.run4(i,:,a_sub);
end
...
%% FIGURE1: PANEL A, REPLicATION ATTEMPT
...
% REPLICATION ATTEMPT
bl_idx = strcmp(my_mm.condition, 'baseline');
cntl_r1_idx = strcmp(my_mm.condition, 'control') & my_mm.run == 1;
cntl_r2_idx = strcmp(my_mm.condition, 'control') & my_mm.run == 2;

my_x1 = mean(my_mm.("delta"){bl_idx})';
my_y1 = [mean(my_mm.("delta"){cntl_r1_idx})', mean(my_mm.("delta"){cntl_r2_idx})'];
my_x2 = mean(my_mm.("theta"){bl_idx})';
my_y2 = [mean(my_mm.("theta"){cntl_r1_idx})', mean(my_mm.("theta"){cntl_r2_idx})'];
my_x3 = mean(my_mm.("alpha"){bl_idx})';
my_y3 = [mean(my_mm.("alpha"){cntl_r1_idx})', mean(my_mm.("alpha"){cntl_r2_idx})'];
my_x4 = mean(my_mm.("beta"){bl_idx})';
my_y4 = [mean(my_mm.("beta"){cntl_r1_idx})', mean(my_mm.("beta"){cntl_r2_idx})'];

subplot(2,2,3)
my_vals2plot = [my_x1,mean(my_y1,2),my_x2,mean(my_y2,2),my_x3,mean(my_y3,2),my_x4,mean(my_y4,2)];
boxplot(my_vals2plot,'PlotStyle','traditional','BoxStyle','filled','OutlierSize',3,'colors',[.7 .3 .3])
[~,t21_me,~,~] = ttest(my_x1,mean(my_y1,2)); 
[~,t22_me,~,~] = ttest(my_x2,mean(my_y2,2)); 
[~,t23_me,~,~] = ttest(my_x3,mean(my_y3,2)); 
[~,t24_me,~,~] = ttest(my_x4,mean(my_y4,2)); 
...
for i = 1:4
    ...
    for j = 1:23      
        ...
        her_b1 = squeeze(her_mm.baseline(i,j,a_sub));
        her_b2 = squeeze(her_mm.condB.run1(i,j,a_sub));
        her_b3 = squeeze(her_mm.condB.run2(i,j,a_sub));
        [her_tb(1,j),her_tb(2,j),~,sts] = ttest(her_b1,mean([her_b2,her_b3],2));
        her_tb(3,j) = sts.tstat;
    end
...
for i = 1:4
    ...
    e = f2(i);  
    for j = 1:23      
        clear my_b1 my_b2 my_b3
        my_b1 = squeeze(my_mm.raw_df{bl_idx}(i,j,a_sub));
        my_b2 = squeeze(my_mm.raw_df{cntl_r1_idx}(i,j,a_sub));
        my_b3 = squeeze(my_mm.raw_df{cntl_r2_idx}(i,j,a_sub));
        [my_tb(1,j),my_tb(2,j),~,sts] = ttest(my_b1,mean([my_b2,my_b3],2));
        my_tb(3,j) = sts.tstat;
    end
    subplot(2,4,i); hold on;
    plot(1:23, my_tb(3,:), 'r', 'LineWidth',2)
    title(sprintf('Con: BS vs P3-4'));
end
