function [wpli_tble,btwn_tbl,pathl_tble,eff_tbl,ecc,rad,strg_tble, modul_tble, clust_w_tbl,deg_w_tbl,deg_bt8_tbl] = getnetwork(test_folder)
...
chan    = 23;
getdeg_weighted = @(wplimat) squeeze(sum(wplimat(1:4,1:23,1:23),2));
getdeg_thresh = @(degmat,thresh) squeeze(degmat(:,thresh,:));
...
for i = 1:length(sub_idx)
    ...
    df = data.(night_str)(1).wpli;
    % Baseline
    if ~isempty(data.(night_str)(1).wpli)
        wpli.baseline(1:4,1:chan,1:chan,i) = df.mat;  
        betw.baseline(1:4,1:chan,i)     = df.betweenness_centrality;
        deg_w.baseline(1:4,1:chan,i)    = getdeg_weighted(df.mat);
        deg_bt8.baseline(1:4,1:chan,i)  = getdeg_thresh(df.degree,8);
        pathl.baseline(1:4,i)           = df.path_length;
        eff.baseline(1:4,i)             = df.efficiency;
        ecc.baseline(1:4,1:chan,i)      = df.eccentricity;
        rad.baseline(1:4,i)             = df.radius;
        strg.baseline(1:4,1:chan,i)     = df.strength;
        modul.baseline(1:4,i)           = df.comms.Q(:,5); % for gamma ~ 1;
        clust.baseline(1:4,1:9,i)       = mean(df.clusteringcoeff,3);
    else
        wpli.baseline(1:4,1:chan,1:chan,i)  =zeros(4,chan,chan);
        betw.baseline(1:4,1:chan,i)         =zeros(4,chan);
        deg_w.baseline(1:4,1:chan,i)        =zeros(4,chan);
        deg_bt8.baseline(1:4,1:chan,i)        =zeros(4,chan);
    ...
    for r = run_list_light(:)'
        ...
        if r > length(data.(night_str))
            ...
            deg_w.    condA.(run_str)(1:4,1:chan,i) =zeros(4,chan);
            deg_bt8.    condA.(run_str)(1:4,1:chan,i) =zeros(4,chan);
            ...
            if ~isempty(data.(night_str)(r).wpli)
                ...
                deg_w.    condA.(run_str)(1:4,1:chan,i)     = getdeg_weighted(wpli.condA.(run_str)(1:4,1:chan,1:chan,i));
                deg_bt8.    condA.(run_str)(1:4,1:chan,i)    = getdeg_thresh(data.(night_str)(r).wpli.degree,8);
    ...
    for r = run_list_control(:)'
        ...
        if r > length(data.(night_str))
            ...
            deg_w.condB.(run_str)(1:4,1:chan,i)          =zeros(4,chan);
            deg_bt8.condB.(run_str)(1:4,1:chan,i)          =zeros(4,chan);
            ...
            if ~isempty(data.(night_str)(r).wpli)
                ...
                deg_w.    condB.(run_str)(1:4,1:chan,i) = getdeg_weighted(wpli.condB.(run_str)(1:4,1:chan,1:chan,i));
                deg_bt8.   condB.(run_str)(1:4,1:chan,i)   = getdeg_thresh(data.(night_str)(r).wpli.degree,8);
...
deg_w_tbl = create_tbl(deg_w,S);
deg_bt8_tbl = create_tbl(deg_bt8,S);
